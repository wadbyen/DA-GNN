...
class DQNAgent:
    ...
    def __init__(self,
        ...
        # Fixed-LR ablation mode: when set, the cosine schedule AND the
        # ReduceLROnPlateau safety net are both disabled, and the learning
        # rate is pinned to self._lr_fixed_value for the whole run. Used by
        # the constant-η control experiments (DA-GNN-Fixed(η)).
        self._lr_fixed = False
        self._lr_fixed_value = None
    ...
    def learn(self, batch_size: Optional[int] = None) -> float:
        ...
        if self.learn_step_counter % 200 == 0 and not self._lr_fixed:
    ...
    def set_fixed_lr(self, lr: float):
     
        self._lr_fixed = True
        self._lr_fixed_value = float(lr)
        for g in self.optimizer.param_groups:
            g['lr'] = float(lr)
        logger.info(f"LR mode: FIXED at {lr:.2e} "
                    "(cosine schedule and ReduceLROnPlateau disabled)")

    def set_lr_for_episode(self, episode_idx: int):
        """Per-episode cosine LR annealing.
        """
        if self._lr_fixed:
            # Re-assert the constant value in case anything nudged it.
            for g in self.optimizer.param_groups:
                g['lr'] = self._lr_fixed_value
            return
