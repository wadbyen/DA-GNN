...
class DQNTrainer:
    ...
    def __init__(self,
        ...
        self.output_dir = f"training_output_{ts}"
    ...
    def train_episode(self, episode_num: int) -> Dict[str, float]:
        ...
        while not done and step_counter < max_steps:
            ...
            try:
                ...
                env_failed_silently = (
                    abs(reward) < 1e-6
                    and info.get('active_flows', 0) > 0
                    and not info.get('flow_rewards')
                )
                if env_failed_silently:
    ...
    def train(self, num_episodes: int = 200, save_interval: int = 50,
        ...
        self.eval_reward_ema: Optional[float] = None
        self.best_combined_score: float = -float('inf')
        ema_alpha = 0.2
        train_ma20_weight = 0.5
        last_best_episode: int = -1
        ...
        for ep in range(num_episodes):
            ...
            recent_rewards = self.training_metrics['episode_rewards'][-20:]
            ma20 = float(np.mean(recent_rewards)) if recent_rewards else 0.0
            ...
            if ep > 50 and spread < 0.15:
                logger.warning(
                    f"[ep {ep+1}] action spread collapsed: {spread:.2f} "
                    f"— agent may be converging to constant policy")
            ...
            if eval_interval > 0 and (ep + 1) % eval_interval == 0:
                try:
                    ...
                    eval_m = self.evaluate(num_episodes=5)
                    eval_r = eval_m.get('rewards', 0.0)

                    # Update EMA of eval reward (initialise on first eval).
                    if self.eval_reward_ema is None:
                        self.eval_reward_ema = eval_r
                    else:
                        self.eval_reward_ema = (
                            ema_alpha * eval_r
                            + (1.0 - ema_alpha) * self.eval_reward_ema
                        )

                   
                    combined_score = (
                        self.eval_reward_ema
                        + train_ma20_weight * ma20
                    )

                    logger.info(
                        f"Eval reward: {eval_r:+.3f}  "
                        f"EMA: {self.eval_reward_ema:+.3f}  "
                        f"MA20: {ma20:+.3f}  "
                        f"Combined: {combined_score:+.3f}  "
                        f"Best: {self.best_combined_score:+.3f}")

                    # Save "best" only when the combined score improves.
                    if combined_score > self.best_combined_score + self.min_delta:
                        self.best_combined_score = combined_score
                        self.best_reward = eval_r   # keep for back-compat
                        last_best_episode = ep + 1
                        self.patience_counter = 0
                        self.save_checkpoint(ep + 1, best=True)
                        logger.info(
                            f"** new best combined: "
                            f"{combined_score:+.3f} "
                            f"(eval EMA {self.eval_reward_ema:+.3f}, "
                            f"train MA20 {ma20:+.3f}) **")
                    else:
        ...
        if last_best_episode > 0:
            ok = self._load_best_checkpoint()
            if ok:
                logger.info(
                    f"Loaded best checkpoint (from ep {last_best_episode}, "
                    f"score={self.best_combined_score:+.3f}) for return")
        else:
            logger.warning(
                "No best checkpoint was saved during training "
                "(combined score never improved past initial); "
    ...
    def _load_best_checkpoint(self) -> bool:
        ...
        q_path = os.path.join(best_dir, 'dqn_model.pth')
        t_path = os.path.join(best_dir, 'target_model.pth')
        if not os.path.exists(q_path):
            return False
        try:
            q_sd = torch.load(q_path, map_location=self.device)
            self.agent.q_network.load_state_dict(q_sd)
            if os.path.exists(t_path):
                t_sd = torch.load(t_path, map_location=self.device)
                self.agent.target_network.load_state_dict(t_sd)
            return True
        except Exception as e:
            logger.error(f"Failed to load best checkpoint from {best_dir}: {e}")
            return False
            ...
            for ep in range(num_episodes):
                ...
                while not done and sc < max_eval_steps:
                    ...
                    try:
                        ...
                        if abs(r) < 1e-6 and info.get('active_flows', 0) > 0 \
                                and not info.get('flow_rewards'):
